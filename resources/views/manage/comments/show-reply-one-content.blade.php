<div class="row">
            ...
            @if($comment->$fieldKey)
                @if(ends_with($fieldKey, '_files'))
                    ...
                    @if($filesArray and is_array($filesArray) and count($filesArray))
                        @section($comment->hashid . '-column-2')
                ...
                @else
                    ...
                    @endisset
                    @section($comment->hashid . '-column-1')
                        <div class="col-xs-12">
    ...
    <div class="col-xs-12">
        <div class="row">
            <div class="col-md-6">
                @yield($comment->hashid . '-column-1')
            </div>
            <div class="col-md-6">
                @yield($comment->hashid . '-column-2')
            </div>
        </div>
    </div>
    @if(
        (is_null($previewComment) and $comment->department_id) or
        ($previewComment->department_id != $comment->department_id)
        )
        <div class="col-xs-12">
            {{ trans('posts.form.referred_to_target', ['target' => $comment->department->title]) }}
        </div>
    @endif
