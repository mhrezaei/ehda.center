...
MHR.persianCalendar = function(b, a) {
	var c = {
		    inputID: "",
		    extraInputID: "",
		    extraInputFormat: "JD",
...
MHR.persianCalendar.prototype.showHidePicker = function(b) {
	...
	if (c = A$(a)) {
		...
		$('div.picker').css('top' , event.pageY);
		$('div.picker').css('left' , event.pageX);
	}
};
MHR.persianCalendar.prototype.createDateTable = function(b, a, c) {
	var d = cE("table"),
	    e, g, f, h, k, j;
	...
	h = l;
	for (k = persianMonthDays(a[0], a[1]) + l; h < k; ++h) 0 == h % 7 && (e = d.insertRow(-1)), g = e.insertCell(-1), f = cE("a"), f.href = "javascript:void(0)", a[2] = h - l + 1, f.className = "weekday", 6 == h % 7 && (f.className += " friday"), c && (a[2] == c[2] && a[1] == c[1] && a[0] == c[0]) && (f.className += " selected"), a[2] == j[2] && (a[1] == j[1] && a[0] == j[0]) && (f.className += " today", f.title += "\u0627\u0645\u0631\u0648\u0632"), f.onclick = function(a, b) {
		return function() {
			a.fillDateField(b);
			return !1
		}
...
MHR.persianCalendar.prototype.changePickerDate = function(b, a, c) {
	var d;
...
MHR.persianCalendar.prototype.showMonthPicker = function(b, a, c) {
	var d, e, g;
	if (A$(this.inputID + "_pDatePicker")) {
		...
		for (d = 0; 12 > d; ++d) g = cE("a"), g.href = "javascript:void(0)", g.appendChild(cTN(PERSIAN_MONTH_NAMES[d])), d == a - 1 && (g.className = "selected"), g.onclick = function(a, b, c) {
			return function(d) {
...
MHR.persianCalendar.prototype.showYearPicker = function(b, a, c) {
	var d, e, g;
	if (A$(this.inputID + "_pDatePicker")) {
		b || (b = window.event);
		(e = A$(lastOpenPDateMonthYearID)) && e.parentNode.removeChild(e);
		...
		e.className = "monthYearPicker";
		e.id = lastOpenPDateMonthYearID = this.inputID + "monthYearPicker";
		for (d = c - 5; d <= c + 5; ++d) g = cE("a"), g.href = "javascript:void(0)", g.appendChild(cTN(enDigitsToFa(d))), d == c && (g.className = "selected"), g.onclick = function(a, b, c) {
			return function(d) {
				a.monthYearPickerClick(d, b, c);
				return !1
			}
...
MHR.persianCalendar.prototype.setMenuPos = function(b, a) {
	var c = getOffset(a);
...
MHR.persianCalendar.prototype.monthYearPickerClick = function(b, a, c) {
	var d, e;
...
MHR.persianCalendar.prototype.normalizeUserInput = function() {
	var b = this.input,
	    a, c = b.value.trim(),
	    c = faDigitsToEn(c);
...
MHR.persianCalendar.prototype.fillDateField = function(b) {
	var a = this.input,
	    c;
...
MHR.persianCalendar.prototype.keyDownHandler = function(b, a) {
	var c;
	b || (b = window.event);
